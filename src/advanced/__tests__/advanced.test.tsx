...
describe("advanced > ", () => {
  ...
  describe("couponUtils", () => {
    describe("applyCouponPrice", () => {
      test("쿠폰이 없으면 가격에 변동이 없어야 한다.", () => {
        expect(couponUtils.applyCouponPrice(1_000)).toBe(1_000);
      });

      test("쿠폰을 적용하면 type에 맞게 금액에서 차감이 되어야 한다.", () => {
        expect(couponUtils.applyCouponPrice(10_000, mockCoupons[0])).toBe(
          5_000
        );
        expect(couponUtils.applyCouponPrice(10_000, mockCoupons[1])).toBe(
          9_000
        );
      });

      test("할인 금액이 가격보다 크면 0을 반환해야 한다.", () => {
        expect(couponUtils.applyCouponPrice(4_000, mockCoupons[0])).not.toBe(
          -1_000
        );
        expect(couponUtils.applyCouponPrice(4_000, mockCoupons[0])).toBe(0);
      });
    });
  });
  ...
  describe("cartUtils", () => {
    describe("getRemainingStock", () => {
      test("상품 20개 중 장바구니에 3개가 담겨져 있으면 17을 반환해야 한다.", () => {
        expect(cartUtils.getRemainingStock(mockProducts[0], mockCarts)).toBe(
          17
        );
      });

      test("상품 20개 중 장바구니에 20개가 담겨져 있으면 0을 재고를 반환해야 한다.", () => {
        expect(cartUtils.getRemainingStock(mockProducts[1], mockCarts)).toBe(0);
      });
    });
  });
});
